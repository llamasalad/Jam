...
export async function onRequestGet({ request, env }) {
  ...
  try {
    ...
    if (r.ok) {
      ...
      if (d.syncedLyrics) {
        return new Response(
          JSON.stringify({ source: "lrclib", type: "synced", lyrics: d.syncedLyrics }),
          { headers: { "Content-Type": "application/json" } }
        );
      }
      if (d.plainLyrics) {
        return new Response(
          JSON.stringify({ source: "lrclib", type: "plain", lyrics: d.plainLyrics }),
          { headers: { "Content-Type": "application/json" } }
        );
      }
    }
  } catch (err) {
    console.error('[lyrics] LRCLIB error:', err.message);
  }

  // STEP 2: Fall back to Genius
  if (!env.GENIUS_TOKEN) {
    console.warn('[lyrics] WARNING: GENIUS_TOKEN not set - skipping Genius fallback');
    
    return new Response(
      JSON.stringify({"source":null,"type":null,"lyrics":null}),
      { headers: { "Content-Type": "application/json" } }
    );
  }
  ...
  try {
    ...
    if (lyrics) {
      return new Response(
        JSON.stringify({ source: "genius", type: "plain", lyrics }),
        { headers: { "Content-Type": "application/json" } }
      );
    }
  } catch (err) {
...
function scrapeGeniusLyrics(html) {
  try {
    ...
    console.error('[lyrics] Scrape error:', err.message);
    return null;
  }
}
